        ...
        while (count < factor_base.length + MARGIN) {
            ...
            for (int i = 0; i < factor_base.length; i++) {
                ...
                if (q_and_rest[1].equals(BigInteger.ZERO)) {
                    ...
                    i--;
                    // if both q is one and r is zero we have found that r^2 can be expressed as a
                    // product of prime powers in the factor_base
                    if (q_and_rest[0].equals(BigInteger.ONE)) {
                        // If exponent vector does not yet exist in result matrix add to result matrix
                        // and add to count
